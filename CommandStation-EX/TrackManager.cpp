...
int TrackManager::getMainCurrentmA() {
  int total = 0;
  FOR_EACH_TRACK(t) {
    if (!(track[t]->getMode() & TRACK_MODE_MAIN)) continue;
    int raw = track[t]->getCurrentRaw(false);
    if (raw < 0) raw = -raw;
    total += track[t]->raw2mA(raw);
  }
  return total;
}

int TrackManager::getProgCurrentmA() {
  FOR_EACH_TRACK(t) {
    if (!(track[t]->getMode() & TRACK_MODE_PROG)) continue;
    int raw = track[t]->getCurrentRaw(false);
    if (raw < 0) raw = -raw;
    return track[t]->raw2mA(raw);
  }
  return 0;
}
