...
// Optional raw command extension hook. Keeping this hook before tokenisation
// lets downstream integrations handle quoted strings without changing DCC-EX.
extern __attribute__((weak)) bool myRawCommand(Print *stream, const char *command);
...
void DCCEXParser::parseOne(Print *stream, byte *com, RingStream * ringStream)
{
    ...
    if (myRawCommand && myRawCommand(stream, reinterpret_cast<const char *>(com)))
        return;
    byte opcode = com[0];
