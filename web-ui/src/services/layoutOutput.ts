...
export const TURNOUT_OUTPUT_MODE_OPTIONS = [
  {
    value: "accessory",
    label: "Basic accessory · <a address 0|1>",
  },
  {
    value: "extended",
    label: "Extended accessory · <A address aspect>",
  },
...
export function sendTurnoutOutput(
  ...
  if (!Number.isInteger(address) || address <= 0 || address > 2048) {
    return false;
  }
  ...
  if (mode === "extended") {
    ...
    const aspect = Math.max(
      0,
      Math.min(255, Math.trunc(Number(rawAspect)))
    );

    return wsApi.setSignalAspect(address, aspect);
  }

  // Legacy layouts may still contain "vpin". The turnout UI no longer
  // exposes it, but keeping this branch avoids breaking an old layout
  // before it is saved with the new Basic/Extended configuration.
  if (mode === "vpin") {
    return wsApi.setVpin(address, physicalValue);
  }
